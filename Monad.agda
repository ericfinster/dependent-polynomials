...
module Monad where
  ...
  -- in SubStrTm I require the existence of a term, but it's not clear that such a term exists
  -- this assumes apriori that B is not empty
  record SubStr {𝕊 𝕋 𝕍 : TyStr} (P : DepPoly 𝕊 𝕋) (Q : DepPoly 𝕋 𝕍) (R : DepPoly 𝕊 𝕍) (μ : P ⊚ Q ⇒ R) : Type₁ where
    ...
    field
      SubStrCtx : {Γ : Ctx 𝕋} {T : Ty 𝕍} (T' : Ty (𝕍 // T)) (t : Tm Q Γ T)
        → Ty ⌈ Γ ⌉
      -- SubStrTy2 : {Γ : Ctx 𝕋} {T : Ty 𝕍} (T' : Ty (𝕍 // T)) (t : Tm Q Γ T) → Ty 𝕍  I think thats needed on the upper levels 
      SubStrTm : {Γ : Ctx 𝕋} {T : Ty 𝕍} (T' : Ty (𝕍 // T)) (t : Tm Q Γ T)
        → Tm (⇑ Q t) ((SubStrCtx T' t) ► ϵ) T'
      Sublift : {Γ : Ctx 𝕋} {Δ : Ctx 𝕊} {T : Ty 𝕍} (T' : Ty (𝕍 // T)) (t : Tm Q Γ T)
        → (σ : Subst P Δ Γ) → (Δ' : Ctx ⌈ Δ ⌉)
        → (t' : Tm (⇑ R (Tm⇒ μ (Γ , σ , t))) Δ' T')
        → Subst ⌈ σ ⌉s Δ' ((SubStrCtx T' t) ► ϵ) 
      LiftEq : {Γ : Ctx 𝕋} {Δ : Ctx 𝕊} {T : Ty 𝕍} (T' : Ty (𝕍 // T)) (t : Tm Q Γ T)
        → (σ : Subst P Δ Γ) → (Δ' : Ctx ⌈ Δ ⌉)
        → (t' : Tm (⇑ R (Tm⇒ μ (Γ , σ , t))) Δ' T')
        → t' ≡ Tm⇒ (⇑⇒ μ (Γ , σ , t)) (((SubStrCtx T' t) ► ϵ) , ((Sublift T' t σ Δ' t') , SubStrTm T' t))
      ⇑-SubStr : {Γ : Ctx 𝕋} {Δ : Ctx 𝕊} {T : Ty 𝕍} (t : Tm Q Γ T) (σ : Subst P Δ Γ)
        → SubStr ⌈ σ ⌉s (⇑ Q t) (⇑ R (Tm⇒ μ (Γ , σ , t))) ((⇑⇒ μ (Γ , σ , t)))

  record GenWknStr {𝕊 𝕋 𝕍 : TyStr} (P : DepPoly 𝕊 𝕋) (Q : DepPoly 𝕋 𝕍) (R : DepPoly 𝕊 𝕍) (μ : P ⊚ Q ⇒ R) : Type₁ where 
    coinductive
    field
      WkAB : (A : Ty 𝕋) (B : Ty 𝕍) → Ty (𝕋 // A)
      Wkt : (A : Ty 𝕋) (B : Ty 𝕍) (t : Tm Q (A ► ϵ) B) → Tm Q (A ► ((WkAB A B) ► ϵ)) B
      WkSub : {Γ : Ctx 𝕊} (A : Ty 𝕋) (B : Ty 𝕍) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A)
        → Subst P Γ (A ► ((WkAB A B) ► ϵ))
      WkEq : {Γ : Ctx 𝕊} (A : Ty 𝕋) (B : Ty 𝕍) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A)
        → (Tm⇒ μ ((A ► ϵ) , (cns Γ A t' ϵ ϵ (● ϵ)) , t)) ≡ 
          (Tm⇒ μ (((A ► ((WkAB A B) ► ϵ))) , ((transport (λ i → Subst P (++-unit-left Γ (~ i)) (A ► ((WkAB A B) ► ϵ))) (WkSub A B t t')) , (Wkt A B t))))
      WkLift : {Γ : Ctx 𝕊} (A : Ty 𝕋) (B : Ty 𝕍) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A)
        → GenWknStr (⇑ P t') (⇑ Q t) (⇑ R (Tm⇒ μ ((A ► ϵ) , ((tmToSubst t') , t)))) {! ⇑⇒ μ ( (A ► ϵ) , (cns Γ A t' ϵ ϵ (● ϵ)) , t )  !}

        
        -- → Tm⇒ μ (((A ► ϵ)) , ((cns Γ A t' ϵ ϵ (● ϵ)) , t)) ≡ 
        -- Tm⇒ μ (((A ► ((WkAB A B) ► ϵ))) , ((transport (λ i → Subst P (++-unit-left Γ (~ i)) (A ► ((WkAB A B) ► ϵ))) σ) , (Wkt A B t))) 

  record WknStr {𝕋 : TyStr} (P : DepPoly 𝕋 𝕋) (Q : DepPoly 𝕋 𝕋) (R : DepPoly 𝕋 𝕋) (μ : P ⊚ Q ⇒ R) : Type₁ where
    coinductive
    field
      WkP : WkPoly Q
      WkAB : (A B : Ty 𝕋) → Ty (𝕋 // A) 
      Wkt : {Γ : Ctx 𝕋} (A B : Ty 𝕋) (t : Tm Q Γ B) → Tm Q (A ► ((WkAB A B) ► ϵ)) B
      WkSub : {Γ : Ctx 𝕋} (A B : Ty 𝕋) (t : Tm Q Γ B) (t' : Tm P Γ A) -- choosing Γ here and not (A ► ϵ) seems to be more akin to a product
        → Tm (⇑ P t') ϵ (WkAB A B)
      -- WkEq : {Γ : Ctx 𝕋} (A B : Ty 𝕋) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A)
      --   → Tm⇒ μ (((A ► ϵ)) , ((cns Γ A t' ϵ ϵ (● ϵ)) , t)) ≡
      --     Tm⇒ μ (((A ► ((WkAB A B) ► ϵ))) , ((transport (λ i → Subst P (++-unit-left Γ (~ i)) (A ► ((WkAB A B) ► ϵ))) (WkSub A B t t')) , (Wkt A B t)))
      -- WkLift : {Γ : Ctx 𝕋} (A B : Ty 𝕋) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A)
      --   → GenWknStr (⇑ P t') (⇑ Q t) (⇑ R (Tm⇒ μ (Γ , tmToSubst t' , t))) (⇑⇒ μ (Γ , tmToSubst t' , t))  

        
        -- → Tm⇒ μ (((A ► ϵ)) , ((cns Γ A t' ϵ ϵ (● ϵ)) , t)) ≡ 
        -- Tm⇒ μ (((A ► ((WkAB A B) ► ϵ))) , ((transport (λ i → Subst P (++-unit-left Γ (~ i)) (A ► ((WkAB A B) ► ϵ))) σ) , (Wkt A B t)))
      -- WkLift : {Γ : Ctx 𝕋} (A B : Ty 𝕋) (t : Tm Q (A ► ϵ) B) (t' : Tm P Γ A) → WknStr {! ⇑ P t'  !} {! ⇑ Q t  !} {!   !} {!   !}   

  -- record WknStr {𝕋 : TyStr} (P : DepPoly 𝕋 𝕋) (Q : DepPoly 𝕋 𝕋) (R : DepPoly 𝕋 𝕋) (μ : P ⊚ Q ⇒ R) : Type₁ where
  --   coinductive
  --   field
  --     WkAB : (A : Ty 𝕋) (B : Ty 𝕋) → Ty (𝕋 // A)
  --     WkSub : (A : Ty 𝕋) (B : Ty 𝕋) (t : Tm Q (A ► ϵ) B) → (t' : Tm P ϵ A) → Tm (⇑ P t') ϵ (WkAB A B)
  ...
  ε-Eq : {𝕋 : TyStr} (M : Monad 𝕋) → (● ϵ) ≡ (Subst⇒ (η M) (idSubst ϵ))
  ε-Eq M = refl
